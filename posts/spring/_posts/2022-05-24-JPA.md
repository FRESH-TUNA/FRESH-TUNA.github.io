...
- 데이터베이스와 개체지향 프로그래밍 언어간의 데이터를 변환하는 기법이다.
자바쪽 표준은 JPA이다.
- ORM은 추상화 계층이다. 최소한의 노력으로 Database를 교체하는것이 가능할수 있다. (장점)
- 수동으로 하는 CRUD 쿼리 작성을 줄일수 있다. (장점)
- FROM 서브쿼리가 JPA 진영에서 아직 미지원하고 있다. (단점)
...
시퀀스나 테이블 전략 사용시 save를 할때 두개의 connection(id채번, 저장)을 가져와서 진행한다.
id채번을 할때 별개의 connection을 사용하여, 다른 save시 id채번할때 막히지 않도록 구현되어있고, 테이블 전략사용시
롤백을 방지 한다.
...
### 전파 전략
Transaction이 중첩되었을때에는 여러가지 전파(Propagation) 전략이 있는데, 기본적으로는 중첩되어있는 transaction은 기존의 부모 transaction과 합쳐진다. (REQUIRED) (만약 부모에 트랜잭션이 없다면 새롭게 트랜잭션이 생성된다.)
그외에도 무조건 새로운 연결객체를 사용하여 트랜잭션을 생성하여 독립적으로 행동하게 하는(REQUIRES_NEW), 새로운 연결을과 트랜잭션을 사용하되 savepoint(checkpoint)를 사용하여, 독립적으로 행동하되 부모가 롤백시 전체가 롤백되는 (NESTED) 등의 전략이 있다.
